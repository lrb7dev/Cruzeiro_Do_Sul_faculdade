...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dropdownBtn = document.querySelector('.dropdown > a.dropbtn');
    ...
    // Dropdown para Teclado
    if (dropdownBtn) {
        // Abrir/Fechar com a tecla Enter
        dropdownBtn.addEventListener('keydown', function (e) {
            if (e.key === 'Enter' || e.key === ' ') { // Adiciona ' ' (Espaço) para mais compatibilidade
                e.preventDefault();
                this.classList.toggle('active'); // Usa uma classe para controlar o estado
                // Atualiza o atributo aria-expanded para leitores de tela
                const isExpanded = this.classList.contains('active');
                this.setAttribute('aria-expanded', isExpanded);

                // Lógica para mostrar/esconder o conteúdo (no mobile)
                const conteudo = this.nextElementSibling;
                if (conteudo) {
                    // No mobile, o JS controla a exibição
                    if (window.innerWidth <= 768) {
                        conteudo.style.display = isExpanded ? 'flex' : 'none';
                    }
                }
            }
        });

        // Remover a classe 'active' ao perder o foco para fechar (acessibilidade)
        dropdownBtn.addEventListener('blur', function () {
            // Um pequeno delay permite que o foco vá para um item interno
            setTimeout(() => {
